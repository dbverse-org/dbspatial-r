# Determine if geometries in two [`dbSpatial`](https://dbverse-org.github.io/dbspatial-r/reference/dbSpatial.md) objects contain each other
...
- name:
  ...
  `character`. Default: "contains_geom"
...
res <- st_contains(x = points,
                   y = points2,
                   overwrite = TRUE)
#> Error in st_contains(x = points, y = points2, overwrite = TRUE): could not find function "st_contains"
